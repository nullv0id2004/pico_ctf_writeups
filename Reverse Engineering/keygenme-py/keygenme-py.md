# Keygenme-py
## Description
[keygenme-trial.py](./keygenme-py_resources/keygenme-trial.py)
## Solving
1) Inspect the python file
```py
#============================================================================#
#============================ARCANE CALCULATOR===============================#
#============================================================================#
...
```
2) I ran it in my terminal to interact with it
3) I observed it wants the licence key to unlock the full version 
4) I entered random text to see the output
5) It denied the access.
6) So I checked how it is checking the license
7) So basically it is compares the length of user entered key to the actual key, if true then it first checks each letter with static key and then it checks the dynamic key by hashing (`b'PRITCHARD'`) and the key being its index [4,5,3,6,2,7,1,8] which is appended to static key.
8) So I hashed the bUsername in terminal and and got the dynamic key by appending the indexes.
```py
import hashlib
...
```
![](./keygenme-py_resources/kg1.png)
9) Now I tried the key in by executing the python code and it worked so this must be the flag.
